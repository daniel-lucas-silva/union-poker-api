...
class PlayersController extends Controller
{
     ...
     * @throws ResponseException
     */
    private function createPasswordRecovery($email)
    {
        $condition = 'email = :email:';
        $parameters = [
            'email' => trim($email)
        ];
        $passwdResets = PasswordResets::findFirst([
            $condition,
            'bind' => $parameters
        ]);

        $passwdResets->token = $this->encrypt($email);

        $this->tryToSaveData($passwdResets, 'common.COULD_NOT_BE_SENT');
    ...
    private function sendRecoveryEmail($email, $token)
    {
        ...
        $this->mailer->Subject = 'Password recover';
        $this->mailer->Body    = 'Code: ' . $token;
     ...
     * @throws ResponseException
     */
    private function setNewPassword(Players $user)
    {
     ...
     * @throws ResponseException
     */
    private function findRecoverRequest($token)
    {
        $condition = 'token = :token: AND email = :email:';
        $parameters = [
            'token' => $token,
            'email' => $this->decrypt($token)
        ];
        $recoverRequest = PasswordResets::findFirst([
            $condition,
            'bind' => $parameters
        ]);
        if ($recoverRequest) return $recoverRequest;

        throw new ResponseException(404, 'users.RECOVER_TOKEN_NOT_FOUND');
    }
     ...
     * @throws Exception
     */
    private function buildTokenData(Players $user)
    {
     ...
     * @throws Exception
     */
    private function registerNewUserAccess(Players $user)
    {
    ...
    public function recoverPassword()
    {
        try {
            ...
            $user = $this->findUser($rawBody['identity']);
            $email = $user->email;
            $passwdResets = $this->createPasswordRecovery($email);
            $this->sendRecoveryEmail($email, $passwdResets->token);
            $this->buildSuccessResponse(200, 'users.EMAIL_RECOVERY_SUCCESSFULLY_SENT');
        } catch (ResponseException $e) {
            $this->buildErrorResponse($e->getCode(), $e->getMessage(), $e->getData());
        } catch (PHPMailerException $e) {
            $this->buildErrorResponse(403, 'users.EMAIL_RECOVERY_NOT_SENT');
        } catch (Exception $e) {
            die($e);
        }
    }
    ...
    public function verifyRecoveryToken()
    {
        try {
            $this->initializePost();
            $rawBody = $this->request->getJsonRawBody(true) ?: [];
            $this->checkForEmptyData($rawBody, ['token']);
            $recoveryRequest = $this->findRecoverRequest($rawBody['token']);

            $this->buildSuccessResponse(200, 'users.RECOVERY_TOKEN_EXISTS', $recoveryRequest->email);
        } catch (ResponseException $e) {
            $this->buildErrorResponse($e->getCode(), $e->getMessage(), $e->getData());
        } catch (Exception $e) {
            die($e);
