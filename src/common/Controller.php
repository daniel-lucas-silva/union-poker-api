...
class Controller extends \Phalcon\Mvc\Controller
{
     ...
     * @return array
     */
    public function buildListingObject(array $elements, $rows, $total)
    {
        $data = [];
        $data = $this->array_push_assoc($data, 'rows_per_page', $rows);
        $data = $this->array_push_assoc($data, 'total_rows', $total);
        $data = $this->array_push_assoc($data, 'rows', $elements);
        return $data;
    }
    ...
    {
//        $model::query()
////        ->innerJoin('Users', 'Videos.user_id = Users.id')
//        ->where($conditions)
//        ->bind($parameters)
//        ->orderBy($order_by)
//        ->limit($limit, $offset);
    ...
    /** Query elements from an specified model
     * @param Model $model
     * @param string $conditions
     * @param array $parameters
     * @param string $columns
     ...
     * @throws ResponseException
     */
    public function queryElements($model, $conditions, $parameters, $columns, $order_by, $offset, $limit)
    {
